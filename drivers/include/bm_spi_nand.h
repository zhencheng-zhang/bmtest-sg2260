...
#ifndef __BM_SPI_NAND_H__
#include "system_common.h"

#define __BM_SPI_NAND_H__

enum WRITE_MODE {
	SPI_NAND_PROGRAM_LOAD_X1 = 0,
	SPI_NAND_PROGRAM_LOAD_X4 = 1,
	WRITE_MODE_NUM,
};

enum READ_MODE {
	SPI_NAND_READ_FROM_CACHE_MODE_X1 = 0,
	SPI_NAND_READ_FROM_CACHE_MODE_X2 = 1,
	SPI_NAND_READ_FROM_CACHE_MODE_X4 = 2,
	READ_MODE_NUM,
};
...
#define FLAGS_ENABLE_X4_BIT	(BIT(2))
#define FLAGS_OW_SETTING_BIT	(BIT(3)) // overwrite setting from sysvec
...
int spi_nand_flash_read_page(int lba, uintptr_t buf, size_t size);
int spi_nand_flash_write_page(uint32_t block_id, uint32_t page_offset, const uintptr_t buf, size_t size);

int bm_spi_nand_flash_program(uint8_t *src_buf, uint32_t base, uint32_t size);
uint16_t bm_spi_nand_read_id(void);
uint32_t bm_spi_nand_get_feature(void);
...
void bm_spi_nand_flash_read_sector(uint32_t addr, uint8_t *buf);
int bm_spi_nand_flash_erase_sector(uint32_t addr);
int bm_spi_nand_flash_program_sector(uint32_t addr);
